...
const { clone } = require("../../main/functions/clone")
const { toValue, actions, queriesClient } = require("../../main/functions/kernel")
const { override } = require("../../main/functions/merge")
...
const getData = ({ _window = {}, req, res, search, action = "search()", verified, collectionProps, unpopulate, checkExistsForSave }) => {
    ...
    if ("doc" in search) {
        ...
        if (fs.existsSync(dbPath(`${path}/collection1/${doc}.json`))) {

            data = { [doc]: JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/${doc}.json`))) }
            single = true
        ...
        for (let index = collectionProps.lastChunk; index > 0; index--) {
            ...
            var chunkProps = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/__props__/chunk${index}/__props__.json`)))
            var docs = chunkProps.docs

            for (let i = 0; i < docs.length; i++) {

                var doc = docs[i]
                var docData = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/${doc}.json`)))
        ...
        while (!chunksRunout && !endSearch) {
            ...
            if ("find" in search) {
                ...
                while (limit > 0 && (i <= docs.length - 1)) {
                    ...
                    if (!skip) data[doc] = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/${doc}.json`)))
                    if (skip) skip--;
                    else limit--;
                    i++;
                }
...
const postData = ({ _window = {}, req, res, save, action = "save()", verified }) => {
    ...
    if (save.rename && save.collection) {
        ...
        fs.renameSync(dbPath(path), dbPath(`bracketDB/${db}/${save.rename}`))

        // edit liveDB collection name
        if (db !== liveDB && save.dev) fs.renameSync(dbPath(`bracketDB/${liveDB}/${save.collection}`), dbPath(`bracketDB/${liveDB}/${save.rename}`))
        ...
        // props
        fs.writeFileSync(dbPath(`bracketDB/${liveDB}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))
        fs.writeFileSync(dbPath(`bracketDB/${liveDB}/__props__/db.json`), JSON.stringify(dbProps, null, 4))

        // change docs collection name
        var docs = fs.readdirSync(dbPath(`bracketDB/${db}/${collection}/collection1`))
        docs.map(doc => {
            if (doc === "__props__") return
            var data = JSON.parse(fs.readFileSync(dbPath(`bracketDB/${db}/${collection}/collection1/${doc}`)))
            data.__props__.collection = save.rename
            fs.writeFileSync(dbPath(`bracketDB/${db}/${collection}/collection1/${doc}`), JSON.stringify(data))
        })
        ...
        if (save.dev) {
            ...
            // change docs collection name
            var docs = fs.readdirSync(dbPath(`bracketDB/${liveDB}/${collection}/collection1`))
            docs.map(doc => {
                if (doc === "__props__") return
                var data = JSON.parse(fs.readFileSync(dbPath(`bracketDB/${liveDB}/${collection}/collection1/${doc}`)))
                data.__props__.collection = save.rename
                fs.writeFileSync(dbPath(`bracketDB/${liveDB}/${collection}/collection1/${doc}`), JSON.stringify(data))
            })
        }
    ...
    for (let i = 0; i < dataList.length; i++) {
        ...
        // recheck props
        dbProps = JSON.parse(fs.readFileSync(dbPath(`${datastore}/${liveDB}/__props__/db.json`)))
        collectionProps = JSON.parse(fs.readFileSync(dbPath(`${datastore}/${liveDB}/${collection}/collection1/__props__/__props__.json`)))
...
const deleteData = ({ _window = {}, req, res, erase, action = "erase()", verified }) => {
    ...
    if (!("docs" in erase) && !("doc" in erase) && !("find" in erase)) {
        ...
        if (fs.existsSync(dbPath(path))) {
            ...
            fs.rmSync(dbPath(path), { recursive: true, force: true })
        ...
        collections.map(coll => {
            if (coll.split(collection)[1] && !coll.split(collection)[0]) {
                ...
                collectionProps = JSON.parse(fs.readFileSync(dbPath(`${datastore}/${liveDB}/${coll}/collection1/__props__/__props__.json`)))
                fs.rmSync(dbPath(`${datastore}/${db}/${coll}`), { recursive: true, force: true })
    ...
    if (docs.length === 0) {
        ...
        docs.map(doc => {
            if (fs.existsSync(dbPath(`${path}/collection1/${doc}.json`))) {
                ...
                // get data
                var docData = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/${doc}.json`))), chunkName = docData.__props__.chunk
                if (!chunks[chunkName]) chunks[chunkName] = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/__props__.json`)))
                ...
                if (collection === "project" && db === bracketDB) {
                    if (fs.existsSync(dbPath(`bracketDB/${docData.db}`))) fs.rmSync(dbPath(`bracketDB/${docData.db}`), { recursive: true, force: true })
                    if (fs.existsSync(dbPath(`bracketDB/${docData.devDB}`))) fs.rmSync(dbPath(`bracketDB/${docData.devDB}`), { recursive: true, force: true })
                }
                ...
                // remove doc
                fs.unlinkSync(dbPath(`${path}/collection1/${doc}.json`))
        ...
        Object.entries(chunks).map(([chunkName, chunkProps]) => {
            ...
            collectionProps.indexes.map(({ doc: indexName }) => {
                var indexing = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/${indexName}.json`)))
                docs.map(doc => delete indexing[doc])
                fs.writeFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/${indexName}.json`), JSON.stringify(indexing, null, 4))
            })

            // save chunk props
            fs.writeFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/__props__.json`), JSON.stringify(chunkProps, null, 4))
        })
    }
...
const checkParams = ({ _window, req, data, action, datastore = "bracketDB" }) => {
    ...
    // props
    if (!fs.existsSync(dbPath(`${datastore}/${liveDB}`))) return ({ success: false, message: "Props error!" })

    // publish
    if (data.publish) return ({ success: true, path })

    if (!fs.existsSync(dbPath(path))) return ({ success: false, message: "Project does not exist!" })

    // collection
    if (collection) {

        path += `/${collection}`
        if (!fs.existsSync(dbPath(path)) && action !== "save()") {
            ...
            if (action === "erase()" && !("docs" in data) && !("doc" in data) && !("find" in data)) {
                ...
                var dbProps = JSON.parse(fs.readFileSync(dbPath(`${datastore}/${liveDB}/__props__/db.json`)))
    ...
    var dbProps = JSON.parse(fs.readFileSync(dbPath(`${datastore}/${liveDB}/__props__/db.json`)))
    var collectionProps = collection && fs.existsSync(dbPath(`${datastore}/${liveDB}/${collection}`)) && JSON.parse(fs.readFileSync(dbPath(`${datastore}/${liveDB}/${collection}/collection1/__props__/__props__.json`)))
    ...
    else if (collectionProps && action === "save()" && liveDB !== db && (!fs.existsSync(dbPath(path)) || getDocNames(path).length === 0)) createCollection({ _window, req, db, collection, liveDB, collectionProps, datastore, dev: true })
    // erase
...
const getFolderNames = path =>
    fs.readdirSync(dbPath(path), { withFileTypes: true })
...
const readProps = ({ collectionProps, dbProps, data, db, collection, datastore = "bracketDB" }) => {
    ...
    fs.writeFileSync(dbPath(`${datastore}/${db}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))

    dbProps.reads += reads
    dbProps.payloadOut += payloadOut
    fs.writeFileSync(dbPath(`${datastore}/${db}/__props__/db.json`), JSON.stringify(dbProps, null, 4))
}
...
const deleteProps = ({ db, collection, collectionProps, dbProps, docsLength = 0, dataSize = 0, datastore = "bracketDB" }) => {
    ...
    // props
    if (fs.existsSync(`${datastore}/${db}/${collection}`)) fs.writeFileSync(dbPath(`${datastore}/${db}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))
    fs.writeFileSync(dbPath(`${datastore}/${db}/__props__/db.json`), JSON.stringify(dbProps, null, 4))
}
...
const publishCollection = ({ _window, req, collection, liveDB, devDB }) => {
    ...
    var collectionProps = JSON.parse(fs.readFileSync(dbPath(`bracketDB/${liveDB}/${collection}/collection1/__props__/__props__.json`)))
    for (let i = collectionProps.lastChunk; i > 0; i--) {
        
        var chunkProps = JSON.parse(fs.readFileSync(dbPath(`bracketDB/${devDB}/${collection}/collection1/__props__/chunk${i}/__props__.json`)))

        // save docs in liveDB and remove from devDB
        for (let j = chunkProps.docs.length - 1; j >= 0; j--) {
            var data = JSON.parse(fs.readFileSync(dbPath(`bracketDB/${devDB}/${collection}/collection1/${chunkProps.docs[j]}.json`)))
...
const publish = ({ _window, req, data }) => {
    ...
    if (!fs.existsSync(dbPath(`bracketDB/${devDB}`) || !fs.existsSync(`bracketDB/${liveDB}`))) return ({ success: false, message: "Wrong DB!" })
    ...
    var collections = fs.readdirSync(dbPath(`bracketDB/${devDB}`))
...
const checkIndexing = ({ global, path, search, finds, chunkName, collectionProps }) => {
    ...
    // get chunk props
    let chunkProps = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/__props__.json`)))
    ...
    for (let index = 0; index < finds.length; index++) {
        ...
        // get index
        indexing = JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/${indexProps.doc}.json`)))

        // read index
        fs.readFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/${indexProps.doc}.json`))
...
const createDB = ({ data }) => {
    ...
    // devDB
    fs.mkdirSync(dbPath(`bracketDB/${data.devDB}`))

    // db
    fs.mkdirSync(dbPath(`bracketDB/${data.db}`))
    fs.mkdirSync(dbPath(`bracketDB/${data.db}/__props__`))
    fs.writeFileSync(dbPath(`bracketDB/${data.db}/__props__/db.json`), JSON.stringify(newProjectProps, null, 4))

    // storage
    fs.mkdirSync(storagePath(`storage/${data.storage}`))
    fs.mkdirSync(storagePath(`storage/${data.storage}/__props__`))
    fs.writeFileSync(storagePath(`storage/${data.storage}/__props__/db.json`), JSON.stringify(newProjectProps, null, 4))

    // cache
    fs.mkdirSync(storagePath(`cache/${data.db}`))
    fs.mkdirSync(storagePath(`cache/${data.devDB}`))
}

const createCollection = ({ _window, req, db, chunkName = "chunk1", collection, liveDB, collectionProps = {}, dev, datastore = "bracketDB" }) => {

    if (fs.existsSync(dbPath(`${datastore}/${liveDB}/${collection}`))) return

    fs.mkdirSync(dbPath(`${datastore}/${liveDB}/${collection}`))
    fs.mkdirSync(dbPath(`${datastore}/${liveDB}/${collection}/collection1`))
    fs.mkdirSync(dbPath(`${datastore}/${liveDB}/${collection}/collection1/__props__`))
    fs.mkdirSync(dbPath(`${datastore}/${liveDB}/${collection}/collection1/__props__/chunk1`))
    fs.writeFileSync(dbPath(`${datastore}/${liveDB}/${collection}/collection1/__props__/chunk1/__props__.json`), JSON.stringify({ docs: [], chunk: "chunk1", docsLength: 0, size: 0 }, null, 4))

    if (datastore === "storage") fs.mkdirSync(dbPath(`${datastore}/${liveDB}/${collection}/bucket1`))
    ...
    // collection props
    fs.writeFileSync(dbPath(`${datastore}/${liveDB}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))
    ...
    fs.writeFileSync(dbPath(`${datastore}/${liveDB}/__props__/db.json`), JSON.stringify(dbProps, null, 4))
    ...
    if (_window.global.manifest.session && (_window.global.manifest.session.devDB || dev)) {
        ...
        if (fs.existsSync(dbPath(`${datastore}/${devDB}/${collection}`))) return

        fs.mkdirSync(dbPath(`${datastore}/${devDB}/${collection}`))
        fs.mkdirSync(dbPath(`${datastore}/${devDB}/${collection}/collection1`))
        fs.mkdirSync(dbPath(`${datastore}/${devDB}/${collection}/collection1/__props__`))
...
const createChunk = ({ db, collection, chunkName, collectionProps, datastore = "bracketDB" }) => {
    ...
    fs.mkdirSync(dbPath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}`))

    // create indexes in chunk1
    if (fs.existsSync(dbPath(`${datastore}/${db}/${collection}`)))
        collectionProps.indexes.map(({ doc }) => fs.writeFileSync(dbPath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}/${doc}.json`), JSON.stringify({}, null, 4)))

    fs.writeFileSync(dbPath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}/__props__.json`), JSON.stringify(chunkProps, null, 4))
    
    for (let i = 0; i < collectionProps.indexes.length; i++) {
        fs.writeFileSync(dbPath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}/${collectionProps.indexes[i].doc}.json`), JSON.stringify({}, null, 4))
    }
}
    ...
    for (let index = 1; index <= collectionProps.lastChunk; index++) {
        ...
        for (let i = 0; i < docs.length; i++) {
            ...
            for (let j = 0; j < searchFields.length; j++) {
                ...
                // get value
                var { success, value, message } = getFieldValue(field.split("."), JSON.parse(fs.readFileSync(dbPath(`${path}/collection1/${doc}.json`))))
                if (!success) break;
        ...
        // save index dev
        fs.writeFileSync(dbPath(`${path}/collection1/__props__/${chunkName}/index${collectionProps.lastIndex}.json`), JSON.stringify(indexing, null, 4))
    }
...
const respond = async ({ res, stack, props, global, response, __ }) => {
    ...
    if (stack && typeof response === "object") {
        ...
        // logs
        console.log((new Date()).getHours() + ":" + (new Date()).getMinutes() + " " + "SEND", executionDuration, global.manifest.session.subdomain || "", global.manifest.session.username || "")
...
const clearProjectCache = ({ db }) => {
    ...
    if (!fs.existsSync(storagePath(`cache/${db}`))) return
    
    getDocNames(storagePath(`cache/${db}`)).map(id => {
        fs.unlinkSync(storagePath(`cache/${db}/${id}`))
    })
}
...
const applySaveSchemaForValue = ({ _window, liveDB, db, key, req, dev, props, data, datastore, collection, action, existingData, save }) => {
    ...
    if (props.type === "boolean") {
        ...
        if (!fs.existsSync(dbPath(`${datastore}/${liveDB}/${props.collection}`))) createCollection({ _window, db, req, dev, collection: props.collection, liveDB, data, datastore })
...
const storagePath = (path) => `../storage/${path}`

const dbserver = async ({ _window, lookupActions, stack, props, address, id, req, res, __, data, object }) => {

    address = actions["addresser()"]({ _window, stack, props, status: "Start", type: "function", nextAddress: address, interpreting: false, asynchronous: true, id, function: "dbserver()", hasWaits: true, object, lookupActions, __, data }).address
