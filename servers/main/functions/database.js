...
const { toValue, actions, queriesClient } = require("./kernel")
...
const database = ({ _window = { global: { manifest: {} } }, req, res, action, preventDefault, data, stack, __ = [], verified, unpopulate, checkExistsForSave }) => {
    ...
    for (let index = 0; index < authorizations.length; index++) {
        ...
        if (action === "search()") {
            ...
            if (data.doc && data.collection && !checkExistsForSave && global.__queries__[data.collection] && global.__queries__[data.collection][data.doc]) queriedData = searchResponse({ data, global })
            else queriedData = getData({ _window, req, res, preventDefault, search: data, action, verified, unpopulate, checkExistsForSave })

        } else if (action === "save()") {
            
            queriedData = postData({ _window, req, res, preventDefault, save: data, action, verified })
        
        } else if (action === "erase()") {
            
            queriedData = deleteData({ _window, req, res, preventDefault, erase: data, action, verified })
        }

        responses.push(queriedData)
    }
...
const saveDB = async ({ _window = { global: { manifest: {} } }, req, res, action, preventDefault, data, stack, __ = [], verified }) => {
    ...
    for (let index = 0; index < authorizations.length; index++) {
        ...
        let data = authorizations[index]
        let queriedData = await fetch("http://localhost:3000", {
            method: "POST", 
            body: JSON.stringify({
    ...
    // end
    let newData = syncData({ global, action, responses, data })
...
const getData = ({ _window = {}, req, res, search, action = "search()", verified, collectionProps, unpopulate, checkExistsForSave }) => {
    ...
    if ("doc" in search) {
        ...
        if (fs.existsSync(filePath(`${path}/collection1/${doc}.json`))) {

            data = { [doc]: JSON.parse(fs.readFileSync(filePath(`${path}/collection1/${doc}.json`))) }
            single = true
        ...
        for (let index = collectionProps.lastChunk; index > 0; index--) {
            ...
            var chunkProps = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/__props__/chunk${index}/__props__.json`)))
            var docs = chunkProps.docs

            for (let i = 0; i < docs.length; i++) {

                var doc = docs[i]
                var docData = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/${doc}.json`)))
        ...
        while (!chunksRunout && !endSearch) {
            ...
            if ("find" in search) {
                ...
                while (limit > 0 && (i <= docs.length - 1)) {
                    ...
                    if (!skip) data[doc] = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/${doc}.json`)))
                    if (skip) skip--;
                    else limit--;
                    i++;
                }
...
const postData = ({ _window = {}, req, res, save, action = "save()", verified }) => {
    ...
    if (save.rename && save.collection) {
        ...
        fs.renameSync(filePath(path), filePath(`bracketDB/${db}/${save.rename}`))

        // edit liveDB collection name
        if (db !== liveDB && save.dev) fs.renameSync(filePath(`bracketDB/${liveDB}/${save.collection}`), filePath(`bracketDB/${liveDB}/${save.rename}`))
        ...
        // props
        fs.writeFileSync(filePath(`bracketDB/${liveDB}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))
        fs.writeFileSync(filePath(`bracketDB/${liveDB}/__props__/db.json`), JSON.stringify(dbProps, null, 4))

        // change docs collection name
        var docs = fs.readdirSync(filePath(`bracketDB/${db}/${collection}/collection1`))
        docs.map(doc => {
            if (doc === "__props__") return
            var data = JSON.parse(fs.readFileSync(filePath(`bracketDB/${db}/${collection}/collection1/${doc}`)))
            data.__props__.collection = save.rename
            fs.writeFileSync(filePath(`bracketDB/${db}/${collection}/collection1/${doc}`), JSON.stringify(data))
        })
        ...
        if (save.dev) {
            ...
            // change docs collection name
            var docs = fs.readdirSync(filePath(`bracketDB/${liveDB}/${collection}/collection1`))
            docs.map(doc => {
                if (doc === "__props__") return
                var data = JSON.parse(fs.readFileSync(filePath(`bracketDB/${liveDB}/${collection}/collection1/${doc}`)))
                data.__props__.collection = save.rename
                fs.writeFileSync(filePath(`bracketDB/${liveDB}/${collection}/collection1/${doc}`), JSON.stringify(data))
            })
        }
    ...
    for (let i = 0; i < dataList.length; i++) {
        ...
        // recheck props
        dbProps = JSON.parse(fs.readFileSync(filePath(`${datastore}/${liveDB}/__props__/db.json`)))
        collectionProps = JSON.parse(fs.readFileSync(filePath(`${datastore}/${liveDB}/${collection}/collection1/__props__/__props__.json`)))
        ...
        if (!chunks[chunkName]) chunks[chunkName] = { props: JSON.parse(fs.readFileSync(filePath(`${path}/collection1/__props__/${chunkName}/__props__.json`))), indexings: {} }
        ...
        for (let i = 0; i < collectionProps.indexes.length; i++) {
            ...
            if (indexProps.expiryDate < new Date().getTime()) {
                ...
                fs.rmSync(filePath(`${path}/collection1/__props__/${chunkName}/${indexProps.doc}.json`))
                continue;
            }
            ...
            if (!sameFieldValue) {
                ...
                // get chunk indexing
                if (!chunks[chunkName].indexings[indexProps.doc]) chunks[chunkName].indexings[indexProps.doc] = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/__props__/${chunkName}/${indexProps.doc}.json`)))
        ...
        // save data
        fs.writeFileSync(filePath(`${path}/collection1/${doc}.json`), JSON.stringify(data, null, 4))
    ...
    Object.entries(chunks).map(([chunkName, chunk]) => {
        fs.writeFileSync(filePath(`${path}/collection1/__props__/${chunkName}/__props__.json`), JSON.stringify(chunk.props, null, 4))
        Object.entries(chunk.indexings).map(([indexingDoc, indexing]) => {
            fs.writeFileSync(filePath(`${path}/collection1/__props__/${chunkName}/${indexingDoc}.json`), JSON.stringify(indexing, null, 4))
        })
    })
...
const deleteData = ({ _window = {}, req, res, erase, action = "erase()", verified }) => {
    ...
    if (!("docs" in erase) && !("doc" in erase) && !("find" in erase)) {
        ...
        if (fs.existsSync(filePath(path))) {
            ...
            fs.rmSync(filePath(path), { recursive: true, force: true })
        ...
        collections.map(coll => {
            if (coll.split(collection)[1] && !coll.split(collection)[0]) {
                ...
                collectionProps = JSON.parse(fs.readFileSync(filePath(`${datastore}/${liveDB}/${coll}/collection1/__props__/__props__.json`)))
                fs.rmSync(filePath(`${datastore}/${db}/${coll}`), { recursive: true, force: true })
    ...
    if (docs.length === 0) {
        ...
        docs.map(doc => {
            if (fs.existsSync(filePath(`${path}/collection1/${doc}.json`))) {
                ...
                // get data
                var docData = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/${doc}.json`))), chunkName = docData.__props__.chunk
                if (!chunks[chunkName]) chunks[chunkName] = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/__props__/${chunkName}/__props__.json`)))
                ...
                if (collection === "project" && db === bracketDB) {
                    if (fs.existsSync(filePath(`bracketDB/${docData.db}`))) fs.rmSync(filePath(`bracketDB/${docData.db}`), { recursive: true, force: true })
                    if (fs.existsSync(filePath(`bracketDB/${docData.devDB}`))) fs.rmSync(filePath(`bracketDB/${docData.devDB}`), { recursive: true, force: true })
                }
                ...
                // remove doc
                fs.unlinkSync(filePath(`${path}/collection1/${doc}.json`))
        ...
        Object.entries(chunks).map(([chunkName, chunkProps]) => {
            ...
            collectionProps.indexes.map(({ doc: indexName }) => {
                var indexing = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/__props__/${chunkName}/${indexName}.json`)))
                docs.map(doc => delete indexing[doc])
                fs.writeFileSync(filePath(`${path}/collection1/__props__/${chunkName}/${indexName}.json`), JSON.stringify(indexing, null, 4))
            })

            // save chunk props
            fs.writeFileSync(filePath(`${path}/collection1/__props__/${chunkName}/__props__.json`), JSON.stringify(chunkProps, null, 4))
        })
    }
...
const checkParams = ({ _window, req, data, action, datastore = "bracketDB" }) => {
    ...
    // props
    if (!fs.existsSync(filePath(`${datastore}/${liveDB}`))) return ({ success: false, message: "Props error!" })

    // publish
    if (data.publish) return ({ success: true, path })

    if (!fs.existsSync(filePath(path))) return ({ success: false, message: "Project does not exist!" })

    // collection
    if (collection) {

        path += `/${collection}`
        if (!fs.existsSync(filePath(path)) && action !== "save()") {
            ...
            if (action === "erase()" && !("docs" in data) && !("doc" in data) && !("find" in data)) {
                ...
                var dbProps = JSON.parse(fs.readFileSync(filePath(`${datastore}/${liveDB}/__props__/db.json`)))
    ...
    var dbProps = JSON.parse(fs.readFileSync(filePath(`${datastore}/${liveDB}/__props__/db.json`)))
    var collectionProps = collection && fs.existsSync(filePath(`${datastore}/${liveDB}/${collection}`)) && JSON.parse(fs.readFileSync(filePath(`${datastore}/${liveDB}/${collection}/collection1/__props__/__props__.json`)))
    ...
    else if (collectionProps && action === "save()" && liveDB !== db && (!fs.existsSync(filePath(path)) || getDocNames(path).length === 0)) createCollection({ _window, req, db, collection, liveDB, collectionProps, datastore, dev: true })
    // erase
...
const hideSecured = ({ global, __ }) => {
    ...
    Object.keys(global.__queries__).map(collection => {
        Object.keys(global.__queries__[collection]).map(doc => {
            ...
            if (data.__props__ && data.__props__.secured && (global.__authorized__[collection] ? !global.__authorized__[collection][doc] : true)) {
...
const getFolderNames = path =>
    fs.readdirSync(filePath(path), { withFileTypes: true })
...
const readProps = ({ collectionProps, dbProps, data, db, collection, datastore = "bracketDB" }) => {
    ...
    fs.writeFileSync(filePath(`${datastore}/${db}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))

    dbProps.reads += reads
    dbProps.payloadOut += payloadOut
    fs.writeFileSync(filePath(`${datastore}/${db}/__props__/db.json`), JSON.stringify(dbProps, null, 4))
}
...
const deleteProps = ({ db, collection, collectionProps, dbProps, docsLength = 0, dataSize = 0, datastore = "bracketDB" }) => {
    ...
    // props
    if (fs.existsSync(`${datastore}/${db}/${collection}`)) fs.writeFileSync(filePath(`${datastore}/${db}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))
    fs.writeFileSync(filePath(`${datastore}/${db}/__props__/db.json`), JSON.stringify(dbProps, null, 4))
}
...
const authorizeDB = ({ _window, global, action, data }) => {
    ...
    let session = global.manifest.session
    let authorizations = []
    ...
    // no session yet
    if (!session) return authorizations

    // devDB => add authorizations
    if (session.dev) {
        authorizations.unshift({ ...data, db: session.devDB, devDB: session.devDB, dev: true, liveDB: session.db })
        if (action === "save" || action === "upload") return authorizations
    }
    authorizations.push({ ...data, db: session.db })

    // special case
    if (data.uncheckPlugins) return authorizations

    // get accessabilities from session
    let accessabilities = session.accessabilities || []
...
const publishCollection = ({ _window, req, collection, liveDB, devDB }) => {
    ...
    var collectionProps = JSON.parse(fs.readFileSync(filePath(`bracketDB/${liveDB}/${collection}/collection1/__props__/__props__.json`)))
    for (let i = collectionProps.lastChunk; i > 0; i--) {
        
        var chunkProps = JSON.parse(fs.readFileSync(filePath(`bracketDB/${devDB}/${collection}/collection1/__props__/chunk${i}/__props__.json`)))

        // save docs in liveDB and remove from devDB
        for (let j = chunkProps.docs.length - 1; j >= 0; j--) {
            var data = JSON.parse(fs.readFileSync(filePath(`bracketDB/${devDB}/${collection}/collection1/${chunkProps.docs[j]}.json`)))
...
const publish = ({ _window, req, data }) => {
    ...
    if (!fs.existsSync(filePath(`bracketDB/${devDB}`) || !fs.existsSync(`bracketDB/${liveDB}`))) return ({ success: false, message: "Wrong DB!" })
    ...
    var collections = fs.readdirSync(filePath(`bracketDB/${devDB}`))
...
const checkIndexing = ({ global, path, search, finds, chunkName, collectionProps }) => {
    ...
    // get chunk props
    let chunkProps = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/__props__/${chunkName}/__props__.json`)))
    ...
    for (let index = 0; index < finds.length; index++) {
        ...
        // get index
        indexing = JSON.parse(fs.readFileSync(filePath(`${path}/collection1/__props__/${chunkName}/${indexProps.doc}.json`)))

        // read index
        fs.readFileSync(filePath(`${path}/collection1/__props__/${chunkName}/${indexProps.doc}.json`))
...
const createDB = ({ data }) => {
    ...
    // devDB
    fs.mkdirSync(filePath(`bracketDB/${data.devDB}`))

    // db
    fs.mkdirSync(filePath(`bracketDB/${data.db}`))
    fs.mkdirSync(filePath(`bracketDB/${data.db}/__props__`))
    fs.writeFileSync(filePath(`bracketDB/${data.db}/__props__/db.json`), JSON.stringify(newProjectProps, null, 4))

    // storage
    fs.mkdirSync(filePath(`storage/${data.storage}`))
    fs.mkdirSync(filePath(`storage/${data.storage}/__props__`))
    fs.writeFileSync(filePath(`storage/${data.storage}/__props__/db.json`), JSON.stringify(newProjectProps, null, 4))

    // cache
    fs.mkdirSync(filePath(`cache/${data.db}`))
    fs.mkdirSync(filePath(`cache/${data.devDB}`))
}

const createCollection = ({ _window, req, db, chunkName = "chunk1", collection, liveDB, collectionProps = {}, dev, datastore = "bracketDB" }) => {

    if (fs.existsSync(filePath(`${datastore}/${liveDB}/${collection}`))) return

    fs.mkdirSync(filePath(`${datastore}/${liveDB}/${collection}`))
    fs.mkdirSync(filePath(`${datastore}/${liveDB}/${collection}/collection1`))
    fs.mkdirSync(filePath(`${datastore}/${liveDB}/${collection}/collection1/__props__`))
    fs.mkdirSync(filePath(`${datastore}/${liveDB}/${collection}/collection1/__props__/chunk1`))
    fs.writeFileSync(filePath(`${datastore}/${liveDB}/${collection}/collection1/__props__/chunk1/__props__.json`), JSON.stringify({ docs: [], chunk: "chunk1", docsLength: 0, size: 0 }, null, 4))

    if (datastore === "storage") fs.mkdirSync(filePath(`${datastore}/${liveDB}/${collection}/bucket1`))
    ...
    // collection props
    fs.writeFileSync(filePath(`${datastore}/${liveDB}/${collection}/collection1/__props__/__props__.json`), JSON.stringify(collectionProps, null, 4))
    ...
    fs.writeFileSync(filePath(`${datastore}/${liveDB}/__props__/db.json`), JSON.stringify(dbProps, null, 4))
    ...
    if (_window.global.manifest.session && (_window.global.manifest.session.devDB || dev)) {
        ...
        if (fs.existsSync(filePath(`${datastore}/${devDB}/${collection}`))) return

        fs.mkdirSync(filePath(`${datastore}/${devDB}/${collection}`))
        fs.mkdirSync(filePath(`${datastore}/${devDB}/${collection}/collection1`))
        fs.mkdirSync(filePath(`${datastore}/${devDB}/${collection}/collection1/__props__`))
...
const createChunk = ({ db, collection, chunkName, collectionProps, datastore = "bracketDB" }) => {
    ...
    fs.mkdirSync(filePath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}`))

    // create indexes in chunk1
    if (fs.existsSync(filePath(`${datastore}/${db}/${collection}`)))
        collectionProps.indexes.map(({ doc }) => fs.writeFileSync(filePath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}/${doc}.json`), JSON.stringify({}, null, 4)))

    fs.writeFileSync(filePath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}/__props__.json`), JSON.stringify(chunkProps, null, 4))
    
    for (let i = 0; i < collectionProps.indexes.length; i++) {
        fs.writeFileSync(filePath(`${datastore}/${db}/${collection}/collection1/__props__/${chunkName}/${collectionProps.indexes[i].doc}.json`), JSON.stringify({}, null, 4))
    }
}
    ...
    for (let index = 1; index <= collectionProps.lastChunk; index++) {
        ...
        for (let i = 0; i < docs.length; i++) {
            ...
            for (let j = 0; j < searchFields.length; j++) {
                ...
                // get value
                var { success, value, message } = getFieldValue(field.split("."), JSON.parse(fs.readFileSync(filePath(`${path}/collection1/${doc}.json`))))
                if (!success) break;
        ...
        // save index dev
        fs.writeFileSync(filePath(`${path}/collection1/__props__/${chunkName}/index${collectionProps.lastIndex}.json`), JSON.stringify(indexing, null, 4))
    }
...
const clearProjectCache = ({ db }) => {
    ...
    if (!fs.existsSync(filePath(`cache/${db}`))) return
    
    getDocNames(`cache/${db}`).map(id => {
        fs.unlinkSync(filePath(`cache/${db}/${id}`))
    })
}
...
const applySaveSchemaForValue = ({ _window, liveDB, db, key, req, dev, props, data, datastore, collection, action, existingData, save }) => {
    ...
    if (props.type === "boolean") {
        ...
        if (!fs.existsSync(filePath(`${datastore}/${liveDB}/${props.collection}`))) createCollection({ _window, db, req, dev, collection: props.collection, liveDB, data, datastore })
...
const dbserver = async ({ _window, lookupActions, stack, props, address, id, req, res, __, data, object }) => {

    address = actions["addresser()"]({ _window, stack, props, status: "Start", type: "function", nextAddress: address, interpreting: true, asynchronous: true, id, action: "dbserver()", hasWaits: true, object, lookupActions, __ }).address
...
console.log(data);
    ...
    let response = await fetch("http://localhost:3000", {method: "POST", body: payload}).then(response => response.json())
    console.log(response);
    
    queriesClient({ _window, response, data })

    // awaits
    return actions["stackManager()"]({ _window, lookupActions, stack, props, id, address, req, res, _: response, __ })
}

const dbserver1 = async ({ _window, data }) => {
    ...
    //let { data: response } = await axios.post("http://localhost:3000", payload)
    let response = await fetch("http://localhost:3000", {method: "POST", body: payload}).then(response => response.json())
